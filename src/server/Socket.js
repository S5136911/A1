module.exports ={
     ...
     const chat = io.of('/chat');
          ...
          socket.on('newroom',(newroom)=>{
               ...
               {
               rooms.push(newroom);
               chat.emit('rommlist',JSON.stringify(rooms));
               }
          });
          socket.on('roomlist',()=>{
               console.log("rooms::");
          ...
          socket.on('numusers',(room)=>{
               ...
               for(i=0;i<socketRoomnum.length;i++){
                    if(socketRoom[i][0]== room){
                         usercont = socketRoomnum[i][1];
                    }
               }
               chat.in(room).emit('numusers',usercont);
          });
          socket.on("joinRoom",(room)=>{
               if(rooms.includes(room)){
                    socket.join(room,()=>{
                    var inroomSocketary = false;
                    for(i=0; i<socketRoom.length;i++){
                    if(socketRoom[i][0] == socket.id){
                         socketRoom[i][1]=room;
                         inroom =true;
   ...
   socket.on("leaveRoom",(room)=>{
     ...
     socket.on('disconnect',()=>{
          ...
          for(let i=0;i<socketRoomnum.length;i++){
               if(socketRoomnum[i][0] == socket.id){
